...
namespace MIIPL.Sockets
{
    ...
    public class SocketClient //: IDisposable
    {
        ...
        public void Send(byte[] data)
        {
            ...
            {
                WriteLogEntry(string.Format("Exception occured in Send : {0}", ex.Message));
                throw ex;
            }
        }
        ...
        private void InternalStartClient()
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    this.WriteLogEntry("Remote ID : " + pID +"IDType :" + this.IDType.ToString());
            ...
            {
                WriteLogEntry(string.Format("Exception occured in 'StartClient' : {0}", e.Message));
            }
        }
        ...
        private void ConnectCallback(IAsyncResult ar)
        {
            try
            {
                ...
                if (handler.Connected)
                {
                    if (this.IDType == IDTypeEnum.Remote)
                    {
                        WriteLogEntry(string.Format("Client connected to {0}", handler.RemoteEndPoint.ToString()));
                        OnConnected(handler.GetRemoteID());
                    }
                    else
                    {
                        WriteLogEntry(string.Format("Client connected to {0}", handler.LocalEndPoint.ToString()));
                        OnConnected(handler.GetLocalID());
                    }
                    WaitForData(handler);
                }
                else
                    WriteLogEntry("Unable to connect to Host");
            }
            catch (ObjectDisposedException e)
            {
                OnUnableToConnect("");
                WriteLogEntry("Error occured in ConnectCallback first : " + e.Message);
            }
            catch (Exception e)
            {
                WriteLogEntry("Error occured in ConnectCallback : " + e.Message);
                OnUnableToConnect("");
            }
        }
        ...
        private void WaitForData(Socket handler)
        {
            ...
            {
                WriteLogEntry(string.Format("Error occured in WaitForData : {0}", e.Message));
            ...
            if (ar == oldAynchResult)
            {
                ...
                try
                {
                    ...
                    if (buff.Length > 0)
                    {
                        ...
                        if (this.IDType == IDTypeEnum.Remote)
                            WriteLogEntry(string.Format("Received << {0} : {1}", handler.GetRemoteID(), Encoding.ASCII.GetString(buff, 0, buff.Length)));
                        else
                            WriteLogEntry(string.Format("Received << {0} : {1}", handler.GetLocalID(), Encoding.ASCII.GetString(buff, 0, buff.Length)));
                        //response));
                ...
                {
                    WriteLogEntry(string.Format("Error occured in OnDataReceived : {0}", e.Message));
                    InternalCloseConnection();
                }
            }
            else
        ...
        private bool InternalSend(byte[] Message)
        {
            ...
            {
                WriteLogEntry("Message length must be > 0");
                return false;
            }
            ...
            if (Client == null)
            {
                WriteLogEntry("Connection not initialized");
                return false;
                //ConnectToServer();
            }
            if (!Client.Connected)
            {
                WriteLogEntry("Must be connected to Send a message");
                return false;
            }
            ...
            try
            {
                ...
                if (this.IDType == IDTypeEnum.Remote)
                {
                    WriteLogEntry(string.Format("Send >> {0} : {1}", Client.GetRemoteID(), Encoding.ASCII.GetString(Message)));
                    //Message = "";
                    OnSendComplete(Client.GetRemoteID());
                }
                else
                {
                    WriteLogEntry(string.Format("Send >> {0} : {1}", Client.GetLocalID(), Encoding.ASCII.GetString(Message)));
                    //Message = "";
            ...
            {
                WriteLogEntry(ex.Message + "Send Message Failed!");
                return false;
            }
        }
        #endregion
